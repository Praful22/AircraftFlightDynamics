...
1. **Rate Gyroscopes**: Measures the body angular velocity or rate of rotation of an object about one or more axes. Hence, $\textit{(p,q,r)}$ is measured by a rate gyroscope which is attached close to the Center of Gravity and is configured accordingly to provide three Euler Angles ``` math (\phi, \theta, \psi)```
2. **Accelerometers** : 
